...
public class App18_RabinKarp_StringMatchingAlgo {
    ...
    // TC : O(N)
    static long creageHashValue(String s, int n) {
        long result = 0;
        for (int i = 0; i < n; i++) {
    ...
    public static void main(String[] args) {
        ...
        long patternHash = creageHashValue(pattern, pattern.length());
        long stringHash = creageHashValue(string, pattern.length()); // checking first 3 substring
